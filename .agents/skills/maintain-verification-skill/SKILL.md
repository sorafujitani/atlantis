...
description: >-
  Periodic pass that keeps a project's verification skill and feature map honest:
  parallel source readers per feature, one live session driving every feature, at
  most one PR of proven corrections. Use for /maintain-verification-skill or
...
A feature map rots the moment the app changes. This skill is the upkeep loop for a skill generated by `/create-verification-skill` (or any project-local verification skill with a feature map under `.agents/skills/verify-*/`). The unit of rigor is the feature, not every sentence: cover every feature file from source and exercise every feature live, without terminalising every bullet.

Align with Atlantis: **prove-it-works** on the real surface, **Opening a PR** for shipping corrections, and brain capture only for durable lessons that would improve a different task.
