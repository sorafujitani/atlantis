...
6. Put the playbook's evidence in the PR body: failing-then-passing output, before/after measurements or media, the checker script and its run. An artifact the reviewer can inspect and re-run beats prose; it buys trust in correctness so review time goes to the code itself.
7. Own follow-up. Watch checks, read review comments, and classify each as fix, dismiss with a concrete reason, or ask when product intent is genuinely ambiguous.
