...
4. Verify each component against its baseline via image diff on the matching surface via **control**. A nonzero diff is a fail; investigate the pixel delta, don't wave it through. a bounded iteration loop per component until the diff is zero.
