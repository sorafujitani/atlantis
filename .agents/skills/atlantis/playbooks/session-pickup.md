...
1. Locate the prior trail with the current harness's session inspector or `ccsession`, a resume note, or a pushed branch. Read metadata and the last messages first, then scan back for decision points. Keep only the reduced timeline in the main thread.
