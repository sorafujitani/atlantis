...
6. **Verify the chain from transcripts, not self-report.** Use the current harness's session inspector or `ccsession` to inspect only the candidate sessions. Look at which files each candidate actually opened. Grade behavior from the artifacts and transcript, never from the candidate's claims.
