...
description: >-
  Generate a project-local verification skill that drives the app the way a user
  does — any language, framework, or platform. Use for /create-verification-skill,
  "make a control skill for this repo", or when a project has no scripted way to
  prove UI/CLI/service behavior.
...
Ground the result in Atlantis principles: **prove-it-works** (real surface, not proxies), **build-the-lever** (helpers for deterministic steps), **encode-lessons-in-structure** (map + scripts beat prose reminders). When Atlantis is active, this skill is what **control** means for that app.
...
- **Launch:** the exact command that starts the app for verification, and how to tell it's ready (a log line, a port answering, a prompt). Include teardown. For a short-lived CLI or TUI there is no server to keep alive: launch means build the binary (or install deps) once, then start each drive in its own isolated PTY, env, or tmux session.
- **Doctor:** one read-only check that answers "is this instance worth driving?" — process up, right version/build, port owned by us, auth valid, disposable data dir. An agent runs this first whenever anything looks off.
...
- **Evidence:** what to capture for a proof and where it goes. Default directory: `<repo>/.atlantis/verify-artifacts/<feature>/` (gitignored via `.atlantis/`). State the proof standards: exercise the real user path, not internal setters or test-only endpoints; capture the action and the resulting state, not just the final screen; verify side effects (files written, rows inserted, messages sent) alongside what's visible; mocks only where a production boundary already isolates the external system. When the safe path is a dry-run or test mode, verify what it actually skips by observing (files, network, git refs) rather than trusting its name.
...
Point the user at `/maintain-verification-skill` (or the `maintain-verification-skill` skill) for keeping the map honest as the app changes. Suggest a cadence only if they ask. Durable gotchas that would improve a different task belong in the brain vault via the reflect path; feature-map gotchas stay next to the recipe.
